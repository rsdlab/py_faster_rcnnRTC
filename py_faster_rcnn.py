...
def detect(net, image, CLASSES, rate):
	# Load the demo image
	im = image.copy()
		...
		#	Image input
			...
			for y in range(height):
				for x in range(width):
					img[y, x, 2] = ord(rawdata[y*width*channels + x*channels + 0])
					img[y, x, 1] = ord(rawdata[y*width*channels + x*channels + 1])
					img[y, x, 0] = ord(rawdata[y*width*channels + x*channels + 2])

			print 'detect.. '
                	# Object recognition from image
			global net
			objParam = detect(net, img,self._CLASSES, self._recognitionRate[0])
			print 'timestamp(sec) : %d' % indata.tm.sec
			...
			for y in range(height):
				for x in range(width):
					list_data.append(chr(image[y,x,2]))
					list_data.append(chr(image[y,x,1]))
					list_data.append(chr(image[y,x,0]))

                        self._d_outObjectParam.data = objParam
			self._d_outImage.data = outdata.data
			self._d_outImage.data.image.raw_data = ''.join(list_data)
